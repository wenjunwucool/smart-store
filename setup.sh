...
# Specify the nvme device bind to uio_pci_generic,Not bind all nvme device to uio_pci_generic.
function configure_nvme_with_bdf {
	bdf=$@
	echo $#
        driver_name=vfio-pci
        if [ -z "$(ls /sys/kernel/iommu_groups)" ]; then
...
function configure_linux_bdf {
	configure_nvme_with_bdf bdf1
}
: ${NRHUGE:=1024}
...
for i in $@;do
	if []
if [ "$username" = "reset" -o "$username" = "config" -o "$username" = "status" -o "$username" = "bdf" ]; then
	mode="$username"
	username=""
fi
